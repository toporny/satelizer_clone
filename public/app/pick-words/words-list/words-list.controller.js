(function() {
  ...
  WordsListCtrl.$inject = ['$scope', '$state', '$stateParams', '$translate', 'common', '$location', '$loading', 'apiUrl', 'toastr', 'availableDictionaries', 'user', 'maxWordsPerPage', 'translatePluginToISO'];

  /*
   * Words List Controller
   */
  function WordsListCtrl($scope, $state, $stateParams, $translate, common, $location, $loading, apiUrl, toastr, availableDictionaries, user, maxWordsPerPage, translatePluginToISO) {
    vm = this;
    ...
    var selected_language = $stateParams['selected_language'];
    var fwfnpu = availableDictionaries[selected_language].free_words_for_not_premium_users;
    ...
    vm.data = {
      ...
      howManyStageIsFreeForNormalUser: ''+Math.floor(fwfnpu/maxWordsPerPage)
    };


    // to do: jesli user jest !premium to przy probie wybrania wyrazu niech pojawi sie komunikat
    ...
    function remember(data) {
      ...
      switch (data.action) {
        ...
        case 'add_word_to_list':
          ...
          // todo: test if user is not premium and word is above that 2000 free words
          // availableDictionaries.free_words_for_not_premium_users
          // show alert that only user premium can do this

          if ( vm.data.currentPage >= vm.data.howManyStageIsFreeForNormalUser ) {
            toastr.warning($translate.instant('WORDS_LIST.THIS_FEATURE_IS_AVAILABLE_ONLY_FOR_PREMIUM_USERS'), { timeOut: 1000 });
            return;
          }
